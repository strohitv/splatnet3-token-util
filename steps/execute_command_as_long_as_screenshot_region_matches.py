...
class ExecuteCommandAsLongAsScreenshotRegionMatches:
	...
	def description(self):
		return ('executes a given command as long as a region in a screenshot matches the template provided. It will wait for [DURATION] milliseconds every time the region matches.'
				...
				f'\n#    - Example 1: "{self.command_name} -f ./base.png -x1 100 -y1 200 -x2 300 -y2 400 -c "wait_s 5" -d 2000 -asp ./compare.png" -co 2 to execute a wait command of 5 seconds and compare the rectangle from (100, 200) to (300, 400) of base.png and compare.png with cutoff 2. If still found, wait for 2 seconds and try again.'
				f'\n#    - Example 2: "{self.command_name} --filename ./base.png --x1 100 --y1 200 --x2 300 --y2 400 --command "wait_s 5" --duration 2000 --actual_screenshot_path ./compare.png" --cutoff 2 to execute a wait command of 5 seconds and compare the rectangle from (100, 200) to (300, 400) of base.png and compare.png with cutoff 2. If still found, wait for 2 seconds and try again.')
