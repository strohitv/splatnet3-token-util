...
class BlockAsLongAsRegionMatches:
	def __init__(self, command_name, app_config: AppConfig):
		...
		self.parser = argparse.ArgumentParser(prog=self.command_name,
											  description='Blocks the execution of the script as long as a specific region on the screen (between points (X1, Y1) to (X2, Y2)) looks similar to the same region on a given template',
		...
		self.parser.add_argument('-d', '--duration', required=False, default=1000,
								 help='The frequency of how often this command should check whether the regions still match. Default: 1000 ms')
		...
		self.introduction = 'This command blocks the execution of the script until a specific region of the emulator screen can not be found anymore. Its biggest use case is to optimize load times, for example waiting for an app icon to not be visible anymore after having tapped it (which means the app is now visible).'
