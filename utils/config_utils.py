...
def load_config(args):
	...
	if args.reinitialize_configs or not os.path.exists(args.config):
		logger.info('Generation of configs was manually triggered.' if args.reinitialize_configs else f'Config file "{args.config}" does not exist.')
		logger.info(f'Attempting to generate config file at "{args.config}"')
		logger.info('')
		...
		if os_name == 'Windows':
			...
			logger.info('Found Linux or an unknown operating system')
		...
		if not os.path.exists(emulator_path):
			logger.info(f'Standard emulator path "{emulator_path}" does not exist, searching $PATH')
			...
			if os.path.exists(emulator_path_new):
				...
				logger.info(f'Found emulator path "{emulator_path}" in $PATH')
			else:
				logger.info(f'Could not find emulator path')

		if not os.path.exists(adb_path):
			logger.info(f'Standard adb path "{adb_path}" does not exist, searching $PATH')
			...
			if os.path.exists(adb_path_new):
				...
				logger.info(f'Found adb path "{adb_path}" in $PATH')
			else:
				logger.info(f'Could not find adb path')

		if not os.path.exists(emulator_path) or not os.path.exists(adb_path):
			logger.info(f'emulator path "{emulator_path}" or adb path "{adb_path}" does not exist, searching $ANDROID_HOME')
			...
			if android_home:
				...
				if os.path.exists(emulator_path_new) and os.path.exists(adb_path_new):
					emulator_path = emulator_path_new
					adb_path = adb_path_new
					logger.info(f'Found emulator path "{emulator_path}" and adb path "{adb_path}" in $ANDROID_HOME')
				else:
					logger.info(f'Could not find emulator path or adb path even though $ANDROID_HOME was set, giving up.')
					logger.info('')
					logger.info('ERROR: please edit config.json and set the locations to the emulator and adb paths manually.')
			else:
				logger.info(f'Could not find emulator path or adb path because $ANDROID_HOME is not set, giving up.')
				logger.info('')
				logger.info('ERROR: please edit config.json and set the locations to the emulator and adb paths manually.')
...
def ensure_scripts_exist(args, app_config: AppConfig):
	...
	if args.reinitialize_configs or not os.path.exists(app_config.run_config.boot_script_path):
		logger.info(f'Attempting to generate boot file at "{app_config.run_config.boot_script_path}"')
		...
		logger.info(f'Successfully generated boot file at "{app_config.run_config.boot_script_path}", please fill it with correct steps and run the application again')
		regenerated = True

	if args.reinitialize_configs or not os.path.exists(app_config.run_config.cleanup_script_path):
		logger.info(f'Attempting to generate boot file at "{app_config.run_config.cleanup_script_path}"')
		...
		logger.info(
			f'Successfully generated cleanup file at "{app_config.run_config.cleanup_script_path}", please fill it with correct steps and run the application again')
		regenerated = True
...
def ensure_template_exists(args, app_config: AppConfig):
	...
	if args.reinitialize_configs or not os.path.exists(app_config.run_config.template_path):
		logger.info(f'Attempting to generate template file at "{app_config.run_config.template_path}"')
		...
		logger.info(f'Successfully generated template file at "{app_config.run_config.template_path}", please fill it with correct values and run the application again')
		regenerated = True
